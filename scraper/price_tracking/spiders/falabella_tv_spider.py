...
class FalabellaTvSpider(scrapy.Spider):
  ...
  def parse(self, response):
    ...
    print "%s link elements read." % tv_link    for link in tv_links:
    ...
    next_page = response.xpath('//div[@id="bul-flecha-derecha"]/a/@href').extract(    if next_page:
  ...
  def parse_tv_info(self, response):
    ...
    if req.status_code == 200:
      ...
      if tv_object:
        ...
        tv_item = {
          'price': tv_object['NORMAL'],
          'internet_price': tv_object['INTERNET'],
          'brand': tv_object['brand'],
        }
        stock = tv_object['stockLevel'].split('=')

        tv_item['stock'] =  stock[1].replace('}','') if stock else ''
        ...
        smart_tb = base_info.xpath(u'//th[text()="Smart TV"]/following-sibling::*/text()').extract()
        tv_item['attributes']['smart_tb'] = smart_tb[0] if smart_tb else ''

        three_d = base_info.xpath(u'//th[text()="3D"]/following-sibling::*/text()').extract()
        tv_item['attributes']['three_d'] = three_d[0] if three_d else ''

        yield TvItem(data = tv_item)
